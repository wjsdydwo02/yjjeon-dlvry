<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            ClientOrder # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            ClientOrder
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <Number label="ShopId" v-model="value.shopId" :editMode="editMode" :inputUI="''"/>
            <String label="ShopName" v-model="value.shopName" :editMode="editMode" :inputUI="''"/>
...
<script>
    ...
    export default {
        name: 'BoundedContext185ClientOrder',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/clientOrders'), this.value)
                        } else {
